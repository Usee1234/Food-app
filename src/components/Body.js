 ...
 function Comp({props}){
  ...
  function filterData(searchText,restaurants){
    ...
    const myfilter=restaurants.filter((value,array)=>{
      if(value?.info?.name.toLowerCase().includes(searchText.toLowerCase())===true ){
      ...
      else if(value?.info?.name.toLowerCase().includes(searchText.toLowerCase())===false){
  ...
  // Resturantlist
      ...
      useEffect(()=>{
       ...
       const callApi=async()=>{
          ...
          const data=await fetch("https://www.swiggy.com/dapi/restaurants/list/v5?lat=26.7999454&lng=75.8138096&page_type=DESKTOP_WEB_LISTING");
          const response=await data.json();
          console.log("via useEffect")
          // console.log(response)
          setAllResturants(response?.data?.cards[2]?.card?.card?.gridElements?.infoWithStyle?.restaurants) 
          setFliteredResturants(response?.data?.cards[2]?.card?.card?.gridElements?.infoWithStyle?.restaurants)
          for(let i=0;i<10;i++){
          // optinal chaining
          // setResturants(response?.data?.cards[2]?.data?.data?.cards[i])
          console.log(response?.data?.cards)
          console.log(response?.data?.cards[5]?.card?.card?.gridElements?.infoWithStyle?.restaurants[i]?.info?.name)            
          console.log(response?.data?.cards[5]?.card?.card?.gridElements?.infoWithStyle?.restaurants[i]?.info?.cuisines)
          console.log(response?.data?.cards[5]?.card?.card?.gridElements?.infoWithStyle?.restaurants[i]?.info?.id)
          console.log(response?.data?.cards[2]?.data?.data?.cards[i]?.data?.cuisines)
          console.log(response?.data?.cards[2]?.data?.data?.cards[i]?.data?.cloudinaryImageId)
          console.log(response?.data?.cards[2]?.data?.data?.cards[i]?.data?.maxDeliveryTime)
          ...
          if(!Allrestaurants){
            return(
              <h1>SORRY WE ARE TRYING TO FETCH RESTURAENTS</h1>
            )
          }
        }
  ...
  which ends up calling the state setter again, which triggers React to call your function again.... and so on. */}
   ...
   // The term 'props' is an abbreviation for 'properties' which refers to the properties of an object.
          return  (
            ...
            <>
            <Link to={"/resturent/"+restaurant?.info?.id} className="link2" //link to attribute mera on click pe event fire karega or activate hoga like html ki tarah or route karega
             key={restaurant?.info?.id}>
            <Resturentcard hotel={restaurant} key={restaurant?.info?.id}/>
            </Link>
